...
- **No Docker-internal hostnames in browser-facing URLs** — Presigned URLs, media URLs, or any URL sent to the frontend MUST use `localhost` (or the public hostname), NEVER Docker service names like `minio:9000`, `redis:6379`, `postgres:5432`. Docker hostnames are unreachable from the browser and will cause silent failures. Use separate config for internal vs public endpoints (e.g., `STORAGE_ENDPOINT` vs `STORAGE_PUBLIC_ENDPOINT`).
- **All API responses must work with null optional fields** — If the frontend sends `null` for optional fields, the backend MUST accept it. Never put `@NotNull` on fields that the frontend may not populate. Test every API endpoint with null optional fields.
