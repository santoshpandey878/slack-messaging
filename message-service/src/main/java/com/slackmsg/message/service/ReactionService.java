...
public class ReactionService {
    ...
    public Reaction addReaction(UUID channelId, UUID messageId, AddReactionRequest req) {
        ...
        // Validate message exists, not deleted, belongs to this channel
        Message message = messageStore.findById(tenantId, messageId)
        ...
        // Fan-out (best-effort, no unread tracking)
        try {
            String payload = WsPayloadBuilder.buildReactionAdded(
    ...
    public void removeReaction(UUID channelId, UUID messageId, String emoji) {
        ...
        // Fan-out (best-effort)
        try {
            String payload = WsPayloadBuilder.buildReactionRemoved(
                    tenantId, channelId, messageId, userId, emoji, objectMapper);
            fanoutService.fanoutEvent(tenantId, channelId, payload, userId, false);
        } catch (Exception e) {
            log.error("Reaction removal fanout failed: {}", e.getMessage());
        }
    ...
    public List<Reaction> getReactions(UUID channelId, UUID messageId) {
        UUID userId = TenantContext.getUserId();
        if (!channelService.isMember(channelId, userId)) {
            throw new SecurityException("Not a member of this channel");
        }
        return reactionRepo.findByMessageIdOrderByCreatedAtAsc(messageId);
    }
}
